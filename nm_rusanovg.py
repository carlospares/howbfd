...
class RusanovG(NumericalMethod):
    ...
    def tend(self, x, u, nm, bdry, funH, initCond, eqn, gw, dx, dt, cf, tloc):
        ...
        for i in range(N+2):
            ...
            (Gl, Gr) = self.flux(u_st, x_st, alpha, eqn)
            fl[:,i] +=  Gl
            fl[:,i+1] += Gr  
        ...
        if cf.funh==FunH.DISC or cf.funh==BUMPD:
            ...
            Ssing=  eqn.S(.5*(u[:,ind-1] + u[:,ind]))*(funH.H(x[ind], tloc)- funH.H(x[ind-1], tloc))/dx#
