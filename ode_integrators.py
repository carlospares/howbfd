...
def adamsbashforth4(eqn, Hx, u, x, i):
    ...
    sumSHx = 0.
    for j in [-4, -3, -2, -1]:
        sumSHx += ab_coeff[j+nsteps]*eqn.S(u[:,i+j])*Hx(x[i+j])

    return sumSHx

def adamsbashforth6(eqn, Hx, u, x, i):
    nsteps= 6
...
def adamsbashforth8(eqn, Hx, u, x, i):
    nsteps= 8
...
def adamsmoulton4(eqn, Hx, u, x, i):
    nsteps= 4
    ab_coeff=[1./24., -5./24., 19./24., 9./24]

    sumSHx = 0.
    for j in [-3, -2, -1, 0]:
        sumSHx += ab_coeff[j+nsteps-1]*eqn.S(u[:,i+j])*Hx(x[i+j])

    return sumSHx

def adamsmoulton6(eqn, Hx, u, x, i):
    nsteps= 6
...
def adamsmoulton8(eqn, Hx, u, x, i):
    nsteps= 8
